...
import { BACKEND_ROOT, REPO_ROOT } from './paths.js';
...
function loadFileConfig(): FileConfig {
  const explicit = process.env.APP_CONFIG_PATH?.trim();
  const candidates = explicit
    ? [path.resolve(explicit)]
    : [path.join(REPO_ROOT, 'config.local.json'), path.join(BACKEND_ROOT, 'config.local.json')];
