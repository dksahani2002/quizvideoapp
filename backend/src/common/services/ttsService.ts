...
import { normalizeOpenAiBaseUrl } from './settingsService.js';

const execAsync = promisify(exec);

/** OpenAI /v1/audio/speech voices only (not macOS names like "Alex"). */
const OPENAI_TTS_VOICES = new Set(['alloy', 'echo', 'fable', 'onyx', 'nova', 'shimmer']);

function pickOpenAiTtsVoice(voice: string | undefined, language: string): string {
  const v = (voice || '').trim().toLowerCase();
  if (v && OPENAI_TTS_VOICES.has(v)) return v;
  return language.toLowerCase().startsWith('en') ? 'alloy' : 'nova';
}
...
export class OpenAITTS implements TTSService {
  ...
  constructor(apiKey: string, model: string = 'tts-1', cacheDir: string, openaiBaseUrl?: string) {
    ...
    this.speechUrl = `${base.replace(/\/$/, '')}/audio/speech`;
  }
  ...
  async generate(text: string, language: string, voice?: string): Promise<string> {
    ...
    // Call OpenAI TTS API (map Settings voices like "Alex" → alloy/nova)
    const selectedVoice = pickOpenAiTtsVoice(voice, language);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `OpenAI TTS API error: ${response.status} ${response.statusText}${errBody ? ` — ${errBody.slice(0, 800)}` : ''}`
        );
      }
