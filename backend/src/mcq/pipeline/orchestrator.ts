...
import { Quiz, SegmentMetadata, RenderConfig } from '../../common/types/index.js';
import { validateQuiz, normalizeQuiz } from '../../common/utils/textSanitizer.js';
...
import { TTSService } from '../../common/services/ttsService.js';
...
export async function renderQuizVideo(
  ...
  try {
    ...
    const { extractDuration } = await import('../../common/utils/ffmpeg.js');
    ...
    try {
      if (config.captions?.enabled !== false) {
        ...
        if (config.captions?.burnIn) {
          const { burnInSubtitles } = await import('../../common/utils/ffmpeg.js');
