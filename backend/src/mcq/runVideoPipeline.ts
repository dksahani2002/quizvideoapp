...
import { BACKEND_ROOT, REPO_ROOT } from "../common/config/paths.js";
...
import { Quiz } from "../common/types/index.js";
...
async function runVideoPipeline() {
  try {
    ...
    const fontFallback = path.join(BACKEND_ROOT, "assets", "fonts", "Montserrat-Bold.ttf");
    ...
    const pipelineTheme = process.env.BACKGROUND_IMAGE?.trim()
      ? {
          backgroundImage: path.resolve(BACKEND_ROOT, process.env.BACKGROUND_IMAGE.trim()),
    ...
    const themeLayerFromEnv = (imgEnv: string | undefined, opEnv: string | undefined) =>
      imgEnv?.trim()
        ? {
            backgroundImage: path.resolve(BACKEND_ROOT, imgEnv.trim()),
            ...(opEnv != null && opEnv !== ""
    ...
    let introVoiceFile = path.join(BACKEND_ROOT, "assets", "audio", "intro_voice.mp3");
    let outroVoiceFile = path.join(BACKEND_ROOT, "assets", "audio", "outro_voice.mp3");
    try {
    ...
    await renderIntroSlide(introFile, TOPIC, {
      ...
      dingFile: path.join(BACKEND_ROOT, "assets", "audio", "ding.mp3"),
    ...
    await renderVideo(quizzes, {
      ...
      tempDir,
      outputDir: path.join(BACKEND_ROOT, "output", "videos"),
      cacheDir: path.join(BACKEND_ROOT, "cache"),
      ttsProvider: tts,
